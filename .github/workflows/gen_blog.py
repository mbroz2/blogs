...
def make_blog(issues, is_beta):
    ...
    for i, issue in enumerate(issues):
        ...
        if not is_beta:
            # Get the link from in between the last ocurrence of <GHA-BLOG-BETA-LINK> and (the following first occurance of) </GHA-BLOG-BETA-LINK> in the issue body
            beta_issue_link = issue['body'].rpartition("<GHA-BLOG-BETA-LINK>")[2].partition("</GHA-BLOG-BETA-LINK>")[0]
